...
class myFunction:
    ...
    def count_indexes():
        ...
        df['positives'] = df['list'].apply(lambda lst: str([num for num in list(eval(lst)) if num >= 0]))
        df['negatives'] = df['list'].apply(lambda lst: str([num for num in list(eval(lst)) if num < 0]))
