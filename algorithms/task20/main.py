...
def main():
    ...
    root.title("Количество прожитых дней")

    texts = ['Создать базу данных', 'Создать таблицу', 'Ввести данные', 'Посчитать дни', 'Вывести данные', 'Экспорт в Excel', 'Очистить таблицу']
    commands = [lambda: (db_name_window(root, connection)),
                lambda: (table_window(root, connection)),
                lambda: (insert_window(root, connection)),
                lambda: (myFunctions.count(connection), messagebox.showinfo(title='Инфо', message='Дни высчитаны и записанны!')),
                lambda: (display_dataframe_table(PandasConnection.read_df())),
                lambda: (PandasConnection.to_excel(f'''SELECT full_name as 'Имя', birthday as 'День рождения', days_alive as 'Дней прожито' FROM {DB_NAME}.{TABLE_NAME};'''), messagebox.showinfo(title='Инфо', message='Успешно сохранено в Excel!')),
...
def table_window(root, connection):
    ...
    insert_btn = tk.Button(table_window, text="Выбрать имя", command=lambda: (connection.execute(f'''CREATE TABLE {DB_NAME}.{TABLE_NAME} (`ID` INT UNSIGNED NOT NULL AUTO_INCREMENT,`full_name` VARCHAR(45) NULL,`birthday` DATE NULL,`days_alive` INT NULL,PRIMARY KEY (`ID`));'''), table_window.destroy(), messagebox.showinfo(title='Инфо', message='Успешно создана таблица!'), display_dataframe_table(PandasConnection.show_tables())))
    insert_btn.pack()
...
def insert_window(root, connection):
    ...
    insert_btn = tk.Button(insert_window, text="Добавить данные", command=lambda: (connection.execute(f'''INSERT INTO {DB_NAME}.{TABLE_NAME} (`full_name`, `birthday`) VALUES ('{buttons[0].get()}', '{buttons[1].get()}');'''), insert_window.destroy()))
    insert_btn.pack()
