...
def main():
    ...
    texts = ['Создать базу данных', 'Создать таблицу', 'Ввести данные', 'Высчитать площадь', 'Вывести данные', 'Экспорт в Excel', 'Очистить таблицу']
    commands = [lambda: (db_name_window(root, connection)),
                lambda: (table_window(root, connection)),
                lambda: (insert_window(root, connection)),
                lambda: (myFunctions.count_area(), messagebox.showinfo(title='Инфо', message='Площадь найдена!')),
                lambda: (display_dataframe_table(PandasConnection.read_df())),
                lambda: (PandasConnection.to_excel(f'''SELECT base1 as 'Основание 1', base2 as 'Основание 2', height as 'Высота', area as 'Площадь' FROM {DB_NAME}.{TABLE_NAME};'''), messagebox.showinfo(title='Инфо', message='Успешно сохранено в Excel!')),
...
def table_window(root, connection):
    ...
    insert_btn = tk.Button(table_window, text="Выбрать имя", command=lambda: (connection.execute(f'''CREATE TABLE {DB_NAME}.{TABLE_NAME} (`ID` INT UNSIGNED NOT NULL AUTO_INCREMENT,`base1` FLOAT(8,2) NULL,`base2` FLOAT(8,2) NULL,`height` FLOAT(8,2) NULL,`area` FLOAT(8,2) NULL,PRIMARY KEY (`ID`));'''), table_window.destroy(), messagebox.showinfo(title='Инфо', message='Успешно создана таблица!'), display_dataframe_table(PandasConnection.show_tables())))
    insert_btn.pack()
...
def insert_window(root, connection):
    ...
    texts = ['Основание 1', 'Основание 2', 'Высота']
    buttons = []
    for text in texts:
    ...
    insert_btn = tk.Button(insert_window, text="Добавить данные", command=lambda: (connection.execute(f'''INSERT INTO `task21`.`main` (`base1`, `base2`, `height`) VALUES ('{buttons[0].get()}', '{buttons[1].get()}', '{buttons[2].get()}');'''), insert_window.destroy()))
    insert_btn.pack()
