...
def main():
    ...
    texts = ['Создать базу данных', 'Создать таблицу', 'Ввести данные', 'Просчитать индексы', 'Вывести данные', 'Экспорт в Excel', 'Очистить таблицу']
    commands = [lambda: (db_name_window(root, connection)),
                lambda: (table_window(root, connection)),
                lambda: (insert_window(root, connection)),
                lambda: (myFunction.count_indexes(), messagebox.showinfo(title='Инфо', message='Индексы посчитаны успешно!')),
                lambda: (display_dataframe_table(PandasConnection.read_df())),
                lambda: (PandasConnection.to_excel('''SELECT list as 'Список', indexes as 'Индексы' FROM task23.main;'''), messagebox.showinfo(title='Инфо', message='Успешно сохранено в Excel!')),
...
def table_window(root, connection):
    ...
    insert_btn = tk.Button(table_window, text="Выбрать имя", command=lambda: (connection.execute(f'''CREATE TABLE `task23`.`main` (`ID` INT UNSIGNED NOT NULL AUTO_INCREMENT,`list` TEXT NULL,`indexes` TEXT NULL,PRIMARY KEY (`ID`));'''), table_window.destroy(), messagebox.showinfo(title='Инфо', message='Успешно создана таблица!'), display_dataframe_table(PandasConnection.show_tables())))
    insert_btn.pack()
...
def insert_window(root, connection):
    ...
    insert_btn = tk.Button(insert_window, text="Добавить данные", command=lambda: (connection.execute(f'''INSERT INTO `task23`.`main` (`list`) VALUES ('{buttons[0].get()}');'''), insert_window.destroy()))
    insert_btn.pack()
