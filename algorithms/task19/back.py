...
class myFunctions:
    ...
    def text_to_matrix(text):
        text_list = [list(map(int, i.split(','))) for i in text.split(';')]
        return np.array(text_list)

    @staticmethod
    def multiply_matrixes(matrix1, matrix2):
        matrix1 = myFunctions.text_to_matrix(matrix1)
        matrix2 = myFunctions.text_to_matrix(matrix2)

        matrix3 = np.dot(matrix1, matrix2)

        return matrix3.tolist()

    @staticmethod
    def multiply_and_enter(connection : Connection):
        df = PandasConnection.read_df()
        matrix1 = df['matrix1'][0]
        matrix2 = df['matrix2'][0]

        matrix3 = myFunctions.multiply_matrixes(matrix1, matrix2)
        connection.execute(f'''UPDATE {DB_NAME}.{TABLE_NAME} SET `matrix3` = '{matrix3}' WHERE (`ID` = '1');''')
