...
class myFunction:
    ...
    def count_changes():
        ...
        df['vowels'] = df['list'].apply(lambda text: ''.join(char for char in text if char in vowels))
        df['consonants'] = df['list'].apply(lambda text: ''.join(char for char in text if char not in vowels))
        df['first20'] = df['list'].apply(lambda x: x[0:20])
        df['last15'] = df['list'].apply(lambda x: x[-15:])
