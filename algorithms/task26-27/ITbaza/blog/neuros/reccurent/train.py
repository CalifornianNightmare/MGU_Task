...
def load_and_prepare_data(db_path='air_quality.db', time_step=100):
...
def train():
    ...
    model.compile(optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=0.001), loss='mean_squared_error')

    # Обучение модели
    checkpoint_path = "mymodel/checkpoint.ckpt"
    cp_callback = ModelCheckpoint(filepath=checkpoint_path, save_weights_only=True, verbose=1)

    model.fit(X_train, y_train, epochs=20, batch_size=64, validation_data=(X_test, y_test), callbacks=[cp_callback])
