...
def main():
    ...
    texts = ['Создать базу данных', 'Создать таблицу', 'Ввести данные', 'Вывести данные', 'Экспорт в Excel', 'Очистить таблицу']
    commands = [lambda: (db_name_window(root, connection)),
                ...
                lambda: (PandasConnection.to_excel(''' ### select statement ### '''), messagebox.showinfo(title='Инфо', message='Успешно сохранено в Excel!')),
...
def table_window(root, connection):
    ...
    insert_btn = tk.Button(table_window, text="Выбрать имя", command=lambda: (connection.execute(f''' ### create table statement ### '''), table_window.destroy(), messagebox.showinfo(title='Инфо', message='Успешно создана таблица!'), display_dataframe_table(PandasConnection.show_tables())))
    insert_btn.pack()
...
def insert_window(root, connection):
    ...
    texts = [' ### insert textbox labels here ### ']
    buttons = []
    for text in texts:
    ...
    insert_btn = tk.Button(insert_window, text="Добавить данные", command=lambda: (connection.execute(f''' ### insert statement ### '''), insert_window.destroy()))
    insert_btn.pack()
